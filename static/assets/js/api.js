...
const go_to_login = (msg = '') => {
    if (msg !== '')
        msg = '?error=' + msg;
    document.querySelector('body').classList.add('form-loading');
...
const bodyLoading = (qs = 'body') => document.querySelector(qs).classList.add('form-loading');
const bodyLoadingDisable = (qs = 'body') => document.querySelector(qs).classList.remove('form-loading');
...
const getFormData = (qs = 'form') => {
    ...
    for (const inputEl of $(`${qs} input, ${qs} select, ${qs} textarea`)) {
        ...
        if (inputEl.type === "radio") {
            let $radio = $(`${qs} [name="${inputEl.name}"]:checked`);
            if ($radio.length) {
                res[inputEl.name] = $radio.first().val()
            } else {
